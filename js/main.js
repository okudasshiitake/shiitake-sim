...
function openBatchModal() {
    ...
    if (harvestInfo) harvestInfo.textContent = `🧺 収穫可能: ${harvestCount}本${!hasWorker ? ' (要人を雇う)' : ''}`;
    if (inoInfo) inoInfo.textContent = `🔬 植菌待ち: ${rawCount}本 / 菌${sporeCount}${!hasWorker ? ' (要人を雇う)' : ''}`;
    if (honFuseInfo) honFuseInfo.textContent = `🔧 本伏せ待ち: ${honFuseCount}本${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}本)`}`;
    if (tenchiInfo) tenchiInfo.textContent = `🔄 天地返し: ${tenchiCount}本${!hasWorker ? ' (要人を雇う)' : ''}`;
