...
// 従業員が対応可能かチェック（本数制限あり）
function canWorkerHandle(count) {
    return gameState.workerCount > 0 && count <= getWorkerCapacity();
}
...
document.addEventListener('visibilitychange', async () => {
    if (document.hidden) {
        ...
        // BGMが有効で、再生中だったなら再開を試行
        const bgmEnabled = isBGMEnabled();
        if ((bgmWasPlaying || bgmEnabled) && bgmAudio && !bgmPlaying) {
            try {
                await bgmAudio.play();
                bgmPlaying = true;
                updateBgmButton();
...
function applyWorkerInoculation(logId, sporeType, isDouble) {
    ...
    const logsToProcess = rawLogs.slice(0, capacity);
    const sporeKey = sporeType === 'premium' ? 'premiumSpores' : 'normalSpores';
