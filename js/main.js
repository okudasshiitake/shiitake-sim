...
function showTutorialStep(stepIndex) {
    ...
    if (step.isIntro) {
        ...
        // 猫を喋る猫に変更＆最前面に＆スキップボタン追加
        const catIcon = document.querySelector('.help-cat-icon');
        if (catIcon) {
            catIcon.src = 'image4.png';
            // 猫の鳴き声を再生（20%音量）
            const catSound = new Audio('cat1.mp3');
            catSound.volume = 0.20;
            catSound.play().catch(() => { });
        }
...
function setupEvents() {
    ...
    if (helpBtn) {
        ...
        helpBtn.addEventListener('touchend', (e) => {
            ...
            if (diff > 30 && !isHidden) {
                // 右スワイプ → 隠す
                helpBtn.classList.add('help-hidden');
                isHidden = true;
            } else if (diff < -30 && isHidden) {
                // 左スワイプ → 戻す
                helpBtn.classList.remove('help-hidden');
                isHidden = false;
