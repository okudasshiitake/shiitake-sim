...
// 従業員が担当可能な本数（1人50本）
function getWorkerCapacity() {
    return (gameState.workerCount || 0) * 50;
}
...
const bgmList = [
    ...
    { file: 'bgm8.mp3', name: 'BGM 8', volume: 0.15 },
    { file: 'bgm9.mp3', name: 'BGM 9', volume: 0.15 },
    { file: 'bgm10.mp3', name: 'BGM 10', volume: 0.15 },
    { file: 'bgm11.mp3', name: 'BGM 11', volume: 0.15 },
    { file: 'bgm12.mp3', name: 'BGM 12', volume: 0.15 },
    { file: 'bgm13.mp3', name: 'BGM 13', volume: 0.15 },
    { file: 'bgm14.mp3', name: 'BGM 14', volume: 0.15 },
    { file: 'bgm15.mp3', name: 'BGM 15', volume: 0.15 }
];
...
// ページ復帰時のBGM自動再開 & タイマー停止/再開
let bgmWasPlaying = false;  // スリープ前にBGMが再生中だったか
let timerWasRunning = false; // スリープ前にタイマーが動いていたか
...
document.addEventListener('visibilitychange', async () => {
    if (document.hidden) {
        ...
        if (bgmAudio && bgmPlaying) {
            bgmAudio.pause();
            bgmPlaying = false;  // 停止状態にする
            console.log('BGM paused (page hidden)');
        }

        // タイマーを停止（時間が進まないようにする）
        timerWasRunning = autoTimer !== null;
        if (autoTimer) {
            clearInterval(autoTimer);
            autoTimer = null;
            console.log('Game timer stopped (page hidden)');
        }

        console.log('Page hidden, bgmWasPlaying:', bgmWasPlaying, 'timerWasRunning:', timerWasRunning);
    } else {
        // ページが再表示された（スリープ復帰、タブ戻りなど）
        console.log('Page visible, bgmWasPlaying:', bgmWasPlaying, 'timerWasRunning:', timerWasRunning);
        ...
        // BGMが再生中だった、またはBGMが有効なら再開を試行
        const bgmEnabled = isBGMEnabled();
        if (bgmWasPlaying && bgmEnabled && bgmAudio) {
            try {
                await bgmAudio.play();
                bgmPlaying = true;
                updateBgmButton();
        ...
        // タイマーを再開（自動進行がONで、ゲームオーバーでない場合）
        // autoTimerが既に存在する場合は二重起動を防ぐ
        if (timerWasRunning && gameState.autoAdvance && !gameState.gameOver && gameState.guidedTutorialDone && !autoTimer) {
            autoTimer = setInterval(() => advance(1), 10000);
            console.log('Game timer resumed (page visible)');
        }
    }
});

// バージョンチェック（GitHub Pagesから最新バージョンを取得）
async function checkForUpdates() {
    try {
        ...
        console.log('バージョンチェックをスキップ:', e.message);
    }
}

function showUpdateNotification(newVersion) {
    $('confirmTitle').textContent = '🔄 アップデートがあります';
    $('confirmMessage').innerHTML = `
        <p>新しいバージョン <strong>v${newVersion}</strong> が利用可能です。</p>
        <p style="font-size:0.85rem; color:#aaa;">現在のバージョン: v${APP_VERSION}</p>
        <p style="margin-top:15px;">ページを更新してください。</p>
    `;
    confirmCallback = () => {
    ...
    if (confirmOk) confirmOk.textContent = '🔄 更新する';
}
...
const tutorialSteps = [
    ...
    { id: 'advance', selector: '#advanceWeek', title: '⏭️ 1週間進めよう！', message: '', actionType: 'click' },
    ...
    { id: 'harvest', selector: '.mushroom-slot.mature', title: '🍄‍🟫 椎茸を収穫しよう！', message: '', actionType: 'click', waitForMushroom: true },
    {
        id: 'harvestSuccess',
        title: '🎉 初めての収穫！',
        message: `
    ...
    { id: 'sell', selector: '#openSell', title: '💰 椎茸を販売しよう！', message: '', actionType: 'click', waitForInventory: true },
    { id: 'confirmSell', selector: '#confirmPacking', title: '💰 販売を確定！', message: '', actionType: 'click', waitForModal: 'packingModal', modalClickHandler: true },
    {
        id: 'sellSuccess',
        title: '🎉 おめでとうございます！',
        message: `
    ...
    { id: 'shop', selector: '#openShop', title: '🛒 仕入れに行こう！', message: '', actionType: 'click', delay: 300 },
    { id: 'buyLog', selector: '.shop-item:first-child', title: '🪵 原木を購入！', message: '', actionType: 'click', waitForModal: 'shopModal', fixedHighlight: true },
    { id: 'buyToolTab', selector: '.shop-tab[data-tab="items"]', title: '🔧 道具タブを開く！', message: '', actionType: 'click', waitForModal: 'shopModal' },
    ...
    { id: 'buySporeTab', selector: '.shop-tab[data-tab="spores"]', title: '🔬 菌タブを開く！', message: '', actionType: 'click', waitForModal: 'shopModal' },
    { id: 'buySpore', selector: '.shop-item:first-child', title: '🔬 菌を購入！', message: '', actionType: 'click', waitForModal: 'shopModal', delay: 300, fixedHighlight: true },
    ...
    { id: 'confirmInoculate', selector: '#confirmInoculate', title: '🔬 作業開始！', message: '', actionType: 'click', waitForModal: 'inoculateModal', modalClickHandler: true, isLast: true },
    {
        id: 'stickyHeaderInfo',
        title: '📌 便利な操作パネル',
        message: `
            <div class="tutorial-intro-content">
                ...
                <p style="font-size:0.8rem;color:#888;">⏸️30秒停止 / +1日 / +1週<br>🔧まとめて管理 / 🛒仕入れ / 🍄‍🟫販売<br>などの操作がすぐにできます！</p>
            </div>
        `,
        isIntro: true,
    ...
    },
    {
        ...
        message: `
            <div class="tutorial-intro-content">
                <p>画面の一番下には</p>
                <p style="font-size:0.8rem;margin:4px 0;">📊 統計（今まで栽培した椎茸を確認）</p>
                <p style="font-size:0.8rem;margin:4px 0;">🛒 公式ECサイト（ほだ木販売）</p>
                <p style="font-size:0.8rem;margin:4px 0;">🎵 BGMをタップで曲を変更</p>
                <p style="font-size:0.75rem;color:#888;margin-top:10px;">など、ぜひチェックしてね！</p>
            </div>
        `,
        isIntro: true,
        showAtCenter: true,
...
function showTutorialStep(stepIndex) {
    ...
    if (!step.noOverlay) {
        ...
        if (step.id === 'soak') hintText = '👆 「浸水」をタップ！';
        else if (step.id === 'harvest') hintText = '👆 椎茸をタップ！';
        else if (step.id === 'advance') hintText = '👆 「1週間進める」をタップ！';
        else if (step.id === 'advanceDay') hintText = '👆 「1日進める」をタップ！';
        else if (step.id === 'sell') hintText = '👆 「椎茸を販売」をタップ！';
        else if (step.id === 'confirmSell') hintText = '👇 「産直で販売」ボタンをタップ！';
        else if (step.id === 'shop') hintText = '👆 「仕入れ」をタップ！';
        else if (step.id === 'buyLog') hintText = '👆 「ナラの原木」をタップ！';
        else if (step.id === 'buyToolTab') hintText = '👆 「道具・雇用」タブをタップ！';
        else if (step.id === 'buyDrill') hintText = '👆 「椎茸ドリル」をタップ！';
        else if (step.id === 'buySporeTab') hintText = '👆 「菌」タブをタップ！';
        else if (step.id === 'buySpore') hintText = '👆 「椎茸菌(普通)」をタップ！';
        else if (step.id === 'inoculate') hintText = '👆 「植菌」をタップ！';
        else if (step.id === 'confirmInoculate') hintText = '👆 「作業開始」ボタンをタップ！';
...
function setupEvents() {
    ...
    safeClick('startGame', async () => {
        ...
        // BGMを自動再生（BGMが有効な場合のみ）
        if (!bgmPlaying && isBGMEnabled()) {
            await startBgm(0);
        }
        gameState.tutorialShown = true;
        saveState();
    ...
    // 公式SNS
    safeClick('openSns', openSns);

    // ランキング
    safeClick('openRanking', showRankingModal);

    // セーブメニュー
    safeClick('openSaveMenu', openSaveMenu);

    safeClick('closeHelp', () => closeModal('helpModal'));

    // 本伏せチュートリアル
    safeClick('closeHonfuseTutorial', closeHonfuseTutorial);
    ...
    safeClick('submitRanking', () => {
        const score = gameState.totalSold || 0;
        const weight = gameState.totalHarvestWeight || 0;
        const days = gameState.day || 0;
        closeModal('gameOverModal');
        showScoreSubmitDialog(score, weight, days);
    });
...
// 本伏せチュートリアル カルーセル
let honfuseCarouselIndex = 0;
const HONFUSE_SLIDES_COUNT = 5;

let honfuseTutorialOpening = false; // 二重呼び出し防止フラグ

function showHonfuseTutorial() {
    // 二重呼び出し防止
    if (honfuseTutorialOpening) return;
    honfuseTutorialOpening = true;

    // ガイドチュートリアルを完全に終了
    tutorialActive = false;
    document.querySelectorAll('.tutorial-overlay').forEach(el => el.remove());
    document.querySelectorAll('#tutorialPopup').forEach(el => el.remove());
    document.querySelectorAll('.tutorial-target').forEach(el => {
        el.classList.remove('tutorial-target');
        el.style.zIndex = '';
        el.style.position = '';
    ...
    honfuseCarouselIndex = 0;
    updateHonfuseCarousel();
    createHonfuseDots();
    openModal('honfuseTutorialModal');

    // フラグをリセット（モーダルが開いた後）
    setTimeout(() => { honfuseTutorialOpening = false; }, 1000);
}

function updateHonfuseCarousel() {
    const slides = $('honfuseSlides');
    if (slides) {
        slides.style.transform = `translateX(-${honfuseCarouselIndex * 100}%)`;
    }
    updateHonfuseDots();
}

function createHonfuseDots() {
    const dotsContainer = $('honfuseDots');
    if (!dotsContainer) return;

    dotsContainer.innerHTML = '';
    for (let i = 0; i < HONFUSE_SLIDES_COUNT; i++) {
        const dot = document.createElement('div');
        dot.className = `carousel-dot ${i === 0 ? 'active' : ''}`;
        dot.onclick = () => {
            honfuseCarouselIndex = i;
            updateHonfuseCarousel();
        };
...
function updateHonfuseDots() {
    const dots = document.querySelectorAll('#honfuseDots .carousel-dot');
    dots.forEach((dot, i) => {
        dot.classList.toggle('active', i === honfuseCarouselIndex);
    });
}

function closeHonfuseTutorial() {
    closeModal('honfuseTutorialModal');
    // チュートリアル完了フラグを立てる
...
window.buyItem = function (itemId) {
    ...
    if (itemId === 'worker') {
        ...
        if (gameState.workerCount >= 3) {
            showToast('👷', '従業員は最大3人までです');
            return;
        }
        gameState.workerCount++;
        if (!gameState.ownedItems.includes('worker')) {
            gameState.ownedItems.push('worker');
        }
        addEvent(`従業員を雇用（${gameState.workerCount}人目）`, 'info');
        showToast('👷', `従業員${gameState.workerCount}人目を雇用！`);
        playSound('buy');
        saveState(); renderShop(); render();
        return;
    }

    if (gameState.ownedItems.includes(itemId)) return;

    // 前提条件チェック
    if (item.requires && !gameState.ownedItems.includes(item.requires)) {
        const reqItem = SHOP_ITEMS.find(i => i.id === item.requires);
        showToast('❌', `${reqItem ? reqItem.name : item.requires}が必要です`);
        return;
    }
...
window.sellLog = function (logId) {
    ...
    showConfirm('🛒 ほだ木を出品', `
        <div style="text-align:left;">
            <p><strong>${log.name}</strong>をネットショップに出品しますか？</p>
            <p style="margin-top:10px;">💰 販売価格: <strong>${priceText}</strong></p>
            <ul style="font-size:0.85rem;margin:10px 0 0 16px;color:#aaa;">
                <li>出品後5日間、毎日売れる可能性あり</li>
                <li>良ほだ: 40%/日、普通ほだ: 20%/日</li>
                <li>売れなかった場合は自動で取り消し</li>
            </ul>
        </div>
    `, () => {
        log.forSale = true;
...
function renderSell() {
    ...
    const hasDryer = gameState.ownedItems.includes('dryer');
    const hasDryableItems = inv.length > 0 || leftover.length > 0;

    if (dryBtn) {
        dryBtn.style.display = hasDryer && hasDryableItems && drying.length === 0 ? 'block' : 'none';
    }
    if (sellDriedBtn) {
...
function startDrying() {
    ...
    const totalCount = inv.length + leftover.length;
    ...
    // 在庫と売れ残りをまとめて乾燥中に移動
    gameState.dryingInventory = [...inv, ...leftover];
    gameState.inventory = [];
...
// まとめて散水（散水設備アイコンから呼び出し）
window.batchWatering = function () {
    if (!gameState.ownedItems.includes('sprinkler')) { showToast('💦', '「散水設備」が必要です'); return; }
    let count = 0;
    gameState.logs.forEach(log => {
    ...
    if (count > 0) {
        ...
        showToast('💦', `${count}本に散水完了！`);
        playSound('water');
        saveState(); render();
    } else {
        showToast('💦', '散水が必要な原木がありません');
    }
};
...
// まとめて本伏せ（従業員機能）
window.batchHonFuse = function () {
    if (!gameState.workerCount || gameState.workerCount < 1) {
        showToast('👷', '従業員を雇用してください');
        return;
    }
    const targetLogs = gameState.logs.filter(log =>
        (log.stage === 'kariFuse' && log.fuseDays >= 45) || log.stage === 'honFuseReady'
    );
    if (targetLogs.length === 0) {
        showToast('🔧', '本伏せ可能な原木がありません');
        return;
    }
    const capacity = getWorkerCapacity();
    const logsToProcess = targetLogs.slice(0, capacity);

    logsToProcess.forEach(log => {
        log.stage = 'maturing';
        log.maturingDays = 0;
        log.tenchiCount = 0;
        log.wateringPenalty = 0;
        log.pestPenalty = 0;
        log.beetlePenalty = 0;
    });

    addEvent(`従業員が${logsToProcess.length}本を本伏せ完了`, 'info');
    showToast('🔧', `${logsToProcess.length}本の本伏せ完了！`);
    playSound('buy');
    closeModal('batchModal');
    saveState(); render();
};
...
// まとめて虫取り（従業員機能）
window.batchRemovePests = function () {
    if (!gameState.workerCount || gameState.workerCount < 1) {
        showToast('👷', '従業員を雇用してください');
        return;
    }
    ...
    if (count > 0) {
        addEvent(`従業員が${count}件の害虫を駆除`, 'info');
        showToast('🪲', `${count}件の害虫を駆除！`);
        playSound('buy');
        closeModal('batchModal');
        saveState(); render();
    } else {
        showToast('🪲', '駆除が必要な害虫がありません');
    }
};

// 従業員による植菌代理（1本植菌したら他の木にも適用）
function applyWorkerInoculation(logId, sporeType, isDouble) {
    if (!gameState.workerCount || gameState.workerCount < 1) return 0;

    // 植菌した木以外の生木を取得
    const rawLogs = gameState.logs.filter(log => log.stage === 'raw' && log.id !== logId);
    if (rawLogs.length === 0) return 0;

    // 従業員の担当可能本数（植菌した1本は除く）
    const capacity = getWorkerCapacity() - 1;
    if (capacity <= 0) return 0;

    let logsToProcess = rawLogs.slice(0, capacity);
    const sporeKey = sporeType === 'premium' ? 'sporesPremium' : 'sporesNormal';
    const sporesNeeded = isDouble ? logsToProcess.length * 2 : logsToProcess.length;

    // 現在月取得
    const month = getMonth();
    const offSeason = month < 1 || month > 5;

    // 菌の在庫チェック
    const available = gameState.shopStock[sporeKey] || 0;
    if (available < sporesNeeded) {
        const canProcess = isDouble ? Math.floor(available / 2) : available;
        if (canProcess === 0) return 0;
        logsToProcess = logsToProcess.slice(0, canProcess); // 処理可能な分だけに制限
    }

    // 植菌処理
    const actualSporesUsed = isDouble ? logsToProcess.length * 2 : logsToProcess.length;
    gameState.shopStock[sporeKey] -= actualSporesUsed;

    logsToProcess.forEach(log => {
        log.stage = 'kariFuse';
        log.fuseDays = 0;
        log.sporeType = sporeType;
        log.inoculatedMonth = month;
        log.inoculatedOffSeason = offSeason;
        if (isDouble) {
...
function batchHarvest() {
    ...
    if (total > 0 || contamCount > 0) {
        if (total > 0) {
            ...
            // 統計データ更新
            if (!gameState.stats) gameState.stats = { totalHarvest: 0, totalSales: 0, totalLogsPlanted: 0, harvestBySize: { small: 0, medium: 0, large: 0, deformed: 0 } };
            gameState.stats.totalHarvest += total;
        }
...
// まとめて植菌（人を雇う必要）- 1本目は通常イベント、残りは従業員が自動処理
function batchInoculate() {
    if (!gameState.workerCount || gameState.workerCount < 1) { showToast('👷', '「人を雇う」を購入してください'); return; }
    const month = getMonth();
    ...
    // 従業員の処理可能数
    const workerCapacity = getWorkerCapacity();
    const willProcess = Math.min(rawLogs.length, workerCapacity);

    // 最初の1本の植菌イベントを開く（残りは従業員が自動処理）
    closeModal('batchModal');

    // 説明トーストを表示
    if (rawLogs.length > 1) {
        showToast('👷', `1本植菌すると従業員が残り${Math.min(rawLogs.length - 1, workerCapacity - 1)}本を処理します`);
    }

    // 最初の原木で植菌イベントを開く
    openInoculate(rawLogs[0].id);
}
...
function openBatchModal() {
    const hasWorker = gameState.workerCount > 0;
    ...
    const honFuseBtn = $('batchHonFuse');
    const pestBtn = $('batchRemovePests');

    // 各ボタンに必要な道具
    // フォークリフト → まとめて浸水
    // 人を雇う → まとめて収穫・植菌・天地返し・本伏せ・虫取り
    // 散水設備 → まとめて散水
    ...
    if (honFuseBtn) honFuseBtn.disabled = !hasWorker;
    if (pestBtn) pestBtn.disabled = !hasWorker;
    ...
    const honFuseCount = gameState.logs.filter(l => (l.stage === 'kariFuse' && l.fuseDays >= 45) || l.stage === 'honFuseReady').length;
    const pestCount = gameState.logs.filter(l => l.mothAvailable || l.beetleAvailable).length;

    // 各行の情報を更新
    const soakInfo = $('batchSoakInfo');
    const harvestInfo = $('batchHarvestInfo');
    const inoInfo = $('batchInoculateInfo');
    const tenchiInfo = $('batchTenchiInfo');
    const waterInfo = $('batchWateringInfo');
    const honFuseInfo = $('batchHonFuseInfo');
    const pestInfo = $('batchPestInfo');

    if (soakInfo) soakInfo.textContent = `💧 浸水可能: ${soakCount}本${!hasForklift ? ' (要フォークリフト)' : ''}`;
    if (harvestInfo) harvestInfo.textContent = `🧺 収穫可能: ${harvestCount}本${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}本)`}`;
    if (inoInfo) inoInfo.textContent = `🔬 植菌待ち: ${rawCount}本 / 菌${sporeCount}${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}本)`}`;
    if (honFuseInfo) honFuseInfo.textContent = `🔧 本伏せ待ち: ${honFuseCount}本${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}本)`}`;
    if (tenchiInfo) tenchiInfo.textContent = `🔄 天地返し: ${tenchiCount}本${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}本)`}`;
    if (waterInfo) waterInfo.textContent = `💦 散水対象: ${waterCount}本${!hasSprinkler ? ' (要散水設備)' : ''}`;
    if (pestInfo) pestInfo.textContent = `🪲 害虫発生: ${pestCount}件${!hasWorker ? ' (要従業員)' : ` (上限${workerCapacity}件)`}`;
...
const ACHIEVEMENTS = [
    ...
    { id: 'harvest500', name: '収穫の鬼', desc: '500個収穫', reward: 3000, check: () => gameState.totalHarvested >= 500 },
    { id: 'harvest1000', name: '椎茸大王', desc: '1000個収穫', reward: 5000, check: () => gameState.totalHarvested >= 1000 },
    ...
    { id: 'sales100000', name: '売上10万円', desc: '売上100,000円達成', reward: 5000, check: () => (gameState.totalSold || 0) >= 100000 },
    { id: 'sales500000', name: '売上50万円', desc: '売上500,000円達成', reward: 10000, check: () => (gameState.totalSold || 0) >= 500000 },
    ...
    { id: 'logs20', name: '原木農場', desc: '20本以上所持', reward: 3000, check: () => gameState.logs.length >= 20 },
    { id: 'firstPlant', name: '初植菌', desc: '初めて植菌を行った', reward: 100, check: () => (gameState.stats?.totalLogsPlanted || 0) >= 1 },
    { id: 'plant10', name: '植菌職人', desc: '10本植菌', reward: 500, check: () => (gameState.stats?.totalLogsPlanted || 0) >= 10 },

    // 品質系
    { id: 'firstDonko', name: '初どんこ', desc: '冬に良質どんこを収穫', reward: 300, check: () => (gameState.stats?.harvestBySize?.large || 0) >= 1 },
    {
        id: 'donko10', name: 'どんこ職人', desc: 'どんこを10個収穫', reward: 1000, check: () => {
            const inv = gameState.inventory || [];
            return inv.filter(i => i.grade === 'donko').length >= 10;
        }
    },
    { id: 'kgHarvest', name: '1kg収穫', desc: '総収穫量1kg達成', reward: 500, check: () => gameState.totalHarvestWeight >= 1000 },
    { id: 'kg10Harvest', name: '10kg収穫', desc: '総収穫量10kg達成', reward: 2000, check: () => gameState.totalHarvestWeight >= 10000 },
    { id: 'kg50Harvest', name: '50kg収穫', desc: '総収穫量50kg達成', reward: 5000, check: () => gameState.totalHarvestWeight >= 50000 },

    // 季節・イベント系
    { id: 'firstSoak', name: '初浸水', desc: '初めて原木を浸水', reward: 50, check: () => gameState.firstActions?.soak },
    { id: 'tenchi', name: '天地返し', desc: '天地返しを行った', reward: 200, check: () => gameState.logs.some(l => (l.tenchiCount || 0) >= 1) },
    {
        id: 'tenchiMaster', name: '天地返しの達人', desc: '天地返しを5回行った', reward: 500, check: () => {
            const total = gameState.logs.reduce((sum, l) => sum + (l.tenchiCount || 0), 0);
            return total >= 5;
        }
    },
    { id: 'pestControl', name: '害虫駆除', desc: '害虫を駆除した', reward: 200, check: () => gameState.logs.some(l => l.pestRemoved) },

    // 経営系
    { id: 'money100k', name: '資金10万円', desc: '所持金10万円達成', reward: 1000, check: () => gameState.totalMoney >= 100000 },
    { id: 'money500k', name: '資金50万円', desc: '所持金50万円達成', reward: 5000, check: () => gameState.totalMoney >= 500000 },
    { id: 'money1m', name: 'ミリオン達成', desc: '所持金100万円達成', reward: 10000, check: () => gameState.totalMoney >= 1000000 },
    { id: 'harvestFreq10', name: '収穫10回', desc: '収穫を10回行った', reward: 300, check: () => (gameState.harvestCount || 0) >= 10 },
    { id: 'harvestFreq50', name: '収穫50回', desc: '収穫を50回行った', reward: 1000, check: () => (gameState.harvestCount || 0) >= 50 },

    // 特殊系
    { id: 'catOwner', name: '猫の恩返し', desc: '迷い猫を保護', reward: 500, check: () => gameState.hasCat },
    { id: 'survivor', name: 'サバイバー', desc: '資金0円から復活', reward: 1000, check: () => gameState.survivedBankruptcy },
    { id: 'okudaMachine', name: 'オクダの力', desc: 'オクダの植菌機を入手', reward: 500, check: () => gameState.ownedItems?.includes('okudaMachine') },
    { id: 'driedShiitake', name: '干し椎茸職人', desc: '干し椎茸を作成', reward: 500, check: () => (gameState.driedInventory?.length || 0) >= 1 },
    { id: 'year1', name: '1年目クリア', desc: '1年間経過', reward: 500, check: () => gameState.day >= 365 },
    { id: 'year2', name: '2年目クリア', desc: '2年間経過', reward: 1000, check: () => gameState.day >= 730 },
    { id: 'gameComplete', name: '3年間完走', desc: '3年間の栽培を完了', reward: 5000, check: () => gameState.day >= 1095 },
];
...
// 従業員の名前変更
window.editWorkerName = function () {
    if (!gameState.ownedItems.includes('worker')) return;
    const currentName = gameState.workerName || '従業員';
    const newName = prompt('従業員の名前を入力してください:', currentName);
    if (newName && newName.trim()) {
        gameState.workerName = newName.trim().substring(0, 10);
        addEvent(`従業員の名前を「${gameState.workerName}」に変更`, 'info');
        showToast('👷', `${gameState.workerName}！`);
        saveState();
        render();
    }
};
...
function openSns() {
    $('confirmTitle').textContent = '📱 公式SNS';
    $('confirmMessage').innerHTML = `
        <p>あわじのきのこや(奥田製作所)の最新情報をチェック！</p>
        <p style="font-size:0.85rem; color:#aaa;">※外部サイトに移動します</p>
        <div style="margin-top:15px; display:flex; flex-direction:column; gap:10px;">
            <button class="btn" style="background:#ff0000; color:#fff;" onclick="window.open('https://m.youtube.com/@okuda_shiitake', '_blank'); closeEcModal();">▶️ YouTube</button>
            <button class="btn" style="background:#e1306c; color:#fff;" onclick="window.open('https://www.instagram.com/okuda_shiitake', '_blank'); closeEcModal();">📷 Instagram</button>
            <button class="btn" style="background:#25d4d4; color:#000;" onclick="window.open('https://www.tiktok.com/@awaji_no_kinokoya_okuda', '_blank'); closeEcModal();">🎵 TikTok</button>
            <button class="btn" style="background:#fff; color:#000;" onclick="window.open('https://www.threads.com/@okuda_shiitake', '_blank'); closeEcModal();">@ Threads</button>
            <button class="btn" style="background:#555; color:#fff;" onclick="window.open('https://x.com/okuda_shiitake', '_blank'); closeEcModal();">𝕏 X</button>
        </div>
    `;
    // OKボタンを無効化（キャンセルのみ使用）
    confirmCallback = null;
    ...
    switch (mode) {
        ...
        case 'aging':
            ...
            return logs.sort((a, b) => {
                ...
                const aAging = aAge > 630 ? 1 : 0;
                const bAging = bAge > 630 ? 1 : 0;
        ...
        case 'quality':
            // 品質順（良 > 普通 > 雑菌 > 失敗 > 未確定）
            return logs.sort((a, b) => {
                const qualityOrder = { 'good': 0, 'normal': 1, 'contaminated': 2, 'failed': 3 };
                const aOrder = a.quality ? qualityOrder[a.quality] ?? 4 : 4;
                const bOrder = b.quality ? qualityOrder[b.quality] ?? 4 : 4;
...
// 初めての10月になった時の栽培おさらいポップアップ
function showFirstOctoberTutorial() {
    $('confirmTitle').textContent = '🍄‍🟫 収穫シーズン到来！';
    $('confirmMessage').innerHTML = `
        <div class="tutorial-intro-content" style="text-align:left;">
            <p style="color:#81c784;font-weight:600;margin-bottom:12px;">🎉 ほだ木が収穫可能になりました！</p>
            
            <p style="font-weight:600;margin-top:12px;">🍄‍🟫 これからの流れ</p>
            <ul style="font-size:0.85rem;margin:4px 0 12px 16px;">
                <li><strong>成熟した椎茸</strong>をタップで収穫</li>
                <li>収穫後は <strong>30日間休養</strong></li>
                <li>休養後は<strong>浸水→発生→収穫</strong>を繰り返す</li>
            </ul>
            
            <p style="font-weight:600;margin-top:12px;">💧 浸水について</p>
            <ul style="font-size:0.85rem;margin:4px 0 12px 16px;">
                <li>休養終了後に「浸水」ボタンが使用可能</li>
                <li>浸水すると<strong>5〜7日後</strong>に椎茸が発生</li>
                <li>夏（7〜9月）は浸水できません</li>
            </ul>
            
            <p style="font-weight:600;margin-top:12px;">💰 販売</p>
            <ul style="font-size:0.85rem;margin:4px 0 12px 16px;">
                <li>在庫は<strong>5日で腐る</strong>ので早めに販売</li>
                <li>産直・卸売・干し椎茸から選択</li>
            </ul>
            
            <p style="font-weight:600;margin-top:12px;">🛒 ほだ木販売</p>
            <ul style="font-size:0.85rem;margin:4px 0 12px 16px;">
                <li>原木カード右上の<strong>💰ボタン</strong>で出品</li>
                <li>良/普通品質のほだ木を販売可能</li>
                <li>出品後5日間、毎日売れる可能性あり</li>
            </ul>
            
            <p style="font-size:0.8rem;color:#aaa;margin-top:16px;text-align:center;">
                📅 ゲームは3年間で終了します。<br>たくさん収穫して売上を伸ばそう！
            </p>
        </div>
    `;
    confirmCallback = null;
