...
const bgmList = [
    { file: 'bgm1.mp3', name: 'BGM 1', volume: 0.15 },
    { file: 'bgm2.mp3', name: 'BGM 2', volume: 0.15 },
    { file: 'bgm3.mp3', name: 'BGM 3', volume: 0.15 },
    { file: 'bgm4.mp3', name: 'BGM 4', volume: 0.15 },
    { file: 'bgm5.mp3', name: 'BGM 5', volume: 0.15 },
    { file: 'bgm6.mp3', name: 'BGM 6', volume: 0.15 },
    { file: 'bgm7.mp3', name: 'BGM 7', volume: 0.25 },
...
function startBgm(index) {
    ...
    const volume = currentBgm.volume || 0.15; // デフォルト15%

    bgmAudio = new Audio(currentBgm.file);
    bgmAudio.loop = true;

    // Web Audio API で音量制御（モバイル対応）
    if (audioCtx) {
        try {
            // 既存のソースを切断
            if (bgmSource) { bgmSource.disconnect(); }
            if (bgmGainNode) { bgmGainNode.disconnect(); }

            bgmSource = audioCtx.createMediaElementSource(bgmAudio);
            bgmGainNode = audioCtx.createGain();
            bgmGainNode.gain.value = volume; // 曲ごとの音量
    ...
    if (!gameState.tutorialShown) {
        ...
        if (difficultyRadio) {
            ...
            if (difficultyRadio.value === 'custom') {
                ...
                money = Math.max(-3000, Math.min(100000, money));
            } else {
