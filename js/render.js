...
function renderStatus() {
    ...
    // 回数が0になったらボタンをグレーアウト
    const advanceDayBtn = $('advanceDay');
    const advanceWeekBtn = $('advanceWeek');
    const toggleAutoBtn = $('toggleAuto');

    if (advanceDayBtn) advanceDayBtn.disabled = gameState.dayButtonUses >= DAY_BUTTON_LIMIT;
    if (advanceWeekBtn) advanceWeekBtn.disabled = gameState.weekButtonUses >= WEEK_BUTTON_LIMIT;
    if (toggleAutoBtn && gameState.autoAdvance) toggleAutoBtn.disabled = gameState.pauseUses >= PAUSE_LIMIT;
    ...
    if (workerStatus) workerStatus.style.display = hasWorker ? 'flex' : 'none';
    const workerNameDisplay = $('workerNameDisplay');
    if (workerNameDisplay && hasWorker) {
        const workerName = gameState.workerName || '従業員';
        workerNameDisplay.textContent = gameState.workerCount > 1 ? `${workerName}×${gameState.workerCount}` : workerName;
    }
...
function renderLogs() {
    ...
    gameState.logs.forEach(log => {
        ...
        const canSoak = log.stage === 'active' && log.restDays === 0 && !log.soaking && (!season.isSummer || hasCooler) && !log.hasSoaked;
        ...
        if (log.stage === 'active' && log.mushrooms && log.mushrooms.length > 0) {
            const slots = [];
            const maxSlots = 8; // 表示スロット上限
            const totalMushrooms = log.mushrooms.length;
            const overflow = totalMushrooms > maxSlots ? totalMushrooms - maxSlots + 1 : 0;
            const displayCount = overflow > 0 ? maxSlots - 1 : totalMushrooms;
                ...
                if (m) {
                    if (m.stage === 'sprout') {
                        if (m.isContaminated || m.type === 'contaminated') {
                            // 雑菌は生えてきた直後でも取り除ける
                            slots.push(`<div class="mushroom-slot sprout contaminated" onclick="harvestMushroom(${log.id}, ${i}, event)" style="cursor:pointer;">🦠</div>`);
                        } else {
            ...
            // 溢れた分は「+N」で表示（クリックで残り全て収穫）
            if (overflow > 0) {
                const matureOverflow = log.mushrooms.slice(displayCount).filter(m => m.stage === 'mature').length;
                slots.push(`<div class="mushroom-slot overflow" onclick="harvestLog(${log.id})" title="${overflow}個の椎茸（クリックで収穫）" style="cursor:pointer;font-size:0.8rem;font-weight:bold;color:#81c784;">+${overflow}</div>`);
            }
        ...
        const nameClickable = ''; // 原木名変更機能を無効化
        ...
        card.innerHTML = `
            <div class="log-header">
                ...
                <div class="log-header-right">
                    ...
                    ${!log.isStarter ? `<button class="btn-delete" onclick="deleteLog(${log.id})" title="処分">🗑️</button>` : ''}
                </div>
            </div>
...
function renderLogActions(log, mature, season) {
    ...
    if (log.stage === 'maturing') {
        ...
        if (log.tenchiAvailable) return `<button class="btn btn-tenchi btn-small" onclick="doTenchi(${log.id})">🔄 天地返し（残${log.tenchiDeadline - gameState.day}日）</button>`;
        if (log.mothAvailable) {
            const mothRemaining = log.mothDeadline - gameState.day;
            if (mothRemaining <= 0) { log.mothAvailable = false; }
            else { return `<button class="btn btn-pest btn-small" onclick="removeMoth(${log.id})">🦋 ${log.mothType}を取り除く（残${mothRemaining}日）</button>`; }
        }
        if (log.beetleAvailable) {
            const beetleRemaining = log.beetleDeadline - gameState.day;
            if (beetleRemaining <= 0) { log.beetleAvailable = false; }
            else { return `<button class="btn btn-pest btn-small" onclick="removeBeetle(${log.id})">🪲 ユミアシゴミムシダマシを取り除く（残${beetleRemaining}日）</button>`; }
        }
    ...
    if (log.stage === 'active' && log.restDays === 0) {
        ...
        if (log.tenchiAvailable) return `<button class="btn btn-tenchi btn-small" onclick="doSummerTenchi(${log.id})">🔄 天地返し（残${log.tenchiDeadline - gameState.day}日）</button>`;
        // 浸水は、浸水中でない＆夏以外＆椎茸がない場合のみ可能（クーラーがあれば夏も可）
        const hasMushrooms = log.mushrooms && log.mushrooms.length > 0;
        const hasCooler = gameState.ownedItems.includes('cooler');
        const canSoak = !log.soaking && (!season.isSummer || hasCooler) && !hasMushrooms;
        return `
